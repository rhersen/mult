...
var ui = {
    ...
    getHighscoreList: function getHighscoreList() {
    ...
    createTable: function createTable(limit) {
    ...
    bindAnswer: function bindAnswer(checkAnswer) {
        $("#answer").focus().keyup(function () {
            checkAnswer($(this).val());
        });
    },

    updateQuestion: function updateQuestion(pair) {
        $('.factor-x').text(pair.x);
        $('.factor-y').text(pair.y);
        $('#answer').val('');
    },

    handleScore: function handleScore(endMillis, elapsed, limit, name) {
        $.ajax({
            type: 'POST',
            url: '/score',
            contentType: 'application/json',
            success: ui.getHighscoreList,
            data: JSON.stringify({
                score: elapsed,
                level: limit,
                name: name,
